...
function computeRanking(data) {
  ...
  data.sort(function (a,b) { return d3.ascending(a.round, b.round); })
    .forEach(function (d) {
      ...
      result.push({
        team:d.teams[0],
        img: "flagsColombia/"+d.teams[0] + ".png",
      ...
      result.push({
        team: d.teams[1],
        img: "flagsColombia/"+d.teams[1] + ".png",
...
function redrawRanges(filteredData) {
  ...
  var classifiedData = [
    {range: [lastRound[0], myBumpChart.isBumpChart() ? lastRound[7] :lastRound[8] ],
      name: "Clasifican",
      anchor: "end",
      color: "#00c70d"
    },
    ...
    //   color: "#e6de42"
    // },
    {range: [lastRound[8],lastRound[lastRound.length-1]],
      name: "Eliminados",
      anchor: "middle",
      color: "#f55a5a"
    }
...
d3.json("torneoAperturaColombia2017.json", function(error, data) {
